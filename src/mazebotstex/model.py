...
class MapEncoder(Module):
    ...
    def __init__(self):
        ...
        self.bias2_h = nn.Parameter(torch.zeros(1, 24, 8, 1))
        self.bias2_w = nn.Parameter(torch.zeros(1, 24, 1, 8))
        self.norm2 = nn.GroupNorm(2, 24)

        # 8x8x24 -> 4x4x48
        self.conv3 = nn.Conv2d(24, 48, 2, 2, bias=False)
        self.bias3_h = nn.Parameter(torch.zeros(1, 48, 4, 1))
        self.bias3_w = nn.Parameter(torch.zeros(1, 48, 1, 4))
        self.norm3 = nn.GroupNorm(1, 48)

        # 4x4x48 -> 1x1xE
        self.conv4 = nn.Conv2d(48, cfg.MAP_ENC_SIZE, 4)
...
if __name__ == '__main__':
    ...
    parser = ArgumentParser(description='VisNet reassembler.')
    parser.add_argument('--model_name', '--visnet_name', type=str, default='vistex', help='VisNet name/ID string.')
    parser.add_argument('--out_dir', type=str, default=None, help='Output directory (defaults to local assets).')
    parser.add_argument('--policy_name', type=str, default=None, help='Input policy name/ID string.')
    parser.add_argument('--new_name', type=str, default=None, help='Output policy name/ID string.')
    parsed = parser.parse_known_args()[0]

    if parsed.policy_name is None:
        vis_dir = os.path.join(cfg.DATA_DIR, parsed.model_name)
        ...
        out_dir = cfg.ASSET_DIR if parsed.out_dir is None else parsed.out_dir

        torch.save(vis_model.state_dict(), os.path.join(out_dir, 'visnet.pt'))
        torch.save(vis_model.enc.state_dict(), os.path.join(out_dir, 'visenc.pt'))
        torch.save(vis_model.mlp.state_dict(), os.path.join(out_dir, 'vismlp.pt'))

    elif parsed.new_name is not None:
        import json

        n_bots_list = []

        for level, n_envs in cfg.ENV_NUM_LVL_PRESETS['prog'].items():
            for _ in range(n_envs):
                n_bots_list.append(cfg.LEVEL_PARAMS[level]['n_bots'])

        model_options = {
            'n_bots_list': n_bots_list,
            'com_mode': cfg.COM_TARGET,
            'com_range': 25.5,
            'msg_noise': 0.1,
            'msg_dropout': 0.1}

        with open(os.path.join(cfg.DATA_DIR, parsed.policy_name, 'meta.json'), 'r') as f:
            meta_data = json.load(f)

        latest_timestamp = max(meta_data.keys())
        meta = meta_data[latest_timestamp]
        model_ver = f'model_{meta["ckpt_ver"]:03d}.pt'

        meta['name'] = parsed.new_name
        meta['model_path'] = os.path.join(cfg.DATA_DIR, parsed.new_name, model_ver)

        os.makedirs(os.path.join(cfg.DATA_DIR, parsed.new_name), exist_ok=True)

        with open(os.path.join(cfg.DATA_DIR, parsed.new_name, 'meta.json'), 'w') as f:
            json.dump({latest_timestamp: meta}, f)

        model = ActorCritic(**model_options).to('cpu')

        model_path = os.path.join(cfg.DATA_DIR, parsed.policy_name, model_ver)
        state = torch.load(model_path, map_location='cpu')
        model.load_state_dict(state)

        encoder_path = os.path.join(cfg.DATA_DIR, parsed.visnet_name, 'visenc.pt')
        decoder_path = os.path.join(cfg.DATA_DIR, parsed.visnet_name, 'vismlp.pt')

        model.visencoder.load_state_dict(torch.load(encoder_path, map_location='cpu'))
        model.visdecoder.load_state_dict(torch.load(decoder_path, map_location='cpu'))

        torch.save(model.state_dict(), os.path.join(cfg.DATA_DIR, parsed.new_name, model_ver))

    else:
        raise ValueError(f'Invalid combination of arguments: {parsed.policy_name}, {parsed.new_name}')
