...
from maze import MazeValidator
...
class Interface(BasicInterface):
    ...
    TORQUE_STATES: 'dict[float, list[float]]' = {
        ...
        np.arctan2(0, -1): [-1., -1., -1., -1.],
        np.arctan2(1, 0): [-1.5, 1.5, -1.5, 1.5],
        np.arctan2(-1, 0): [1.5, -1.5, 1.5, -1.5],
    ...
    OFFSET_CAM_AHEAD = gymapi.Vec3(*cfg.CAM_TARGET_OFFSET)
    OFFSET_FLW_BEHIND = gymapi.Vec3(*cfg.FLW_OFFSET)
    ...
    ENT_CLASSES = np.array(cfg.ENT_CLS_CLRS, dtype=np.float32)[:, None, None]
    RGB_CLASSES = np.array(cfg.CLR_CLS_CLRS, dtype=np.float32)[:, None, None]
    ...
    def __init__(self, session: 'Session', sim: MazeSim, device: str):
        ...
        self.path_line_offset = np.array((self.DIR_LINE_OFFSET,), dtype=np.float32)
        self.path_line_clr = np.array((self.PATH_LINE_CLR,), dtype=np.float32)
        self.validator = MazeValidator(env.sampler, env.data)
        ...
        if session.preview:
            ...
            self.visnet.load_state_dict(torch.load(
                os.path.join(self.session.visnet_dir, 'visnet.pt'),
                map_location=device))
    ...
    # --------------------------------------------------------------------------
    # MARK: update_path_lines

    def update_path_lines(self):
        env = self.sim.envs[self.env_idx]

        start_pt = self.session.bot_pos_arr[self.all_bot_idx]
        end_pt = self.session.goal_pos_arr[self.all_bot_idx]

        path_pts = self.validator.get_path(start_pt, end_pt)[0].astype(np.float32)
        path_start_pts = path_pts[:-1]
        path_end_pts = path_pts[1:]

        n_lines = len(path_start_pts)
        path_line_offset = np.broadcast_to(self.path_line_offset, (n_lines, 1))
        ...
        self.gym.add_lines(
            ...
            n_lines,
            np.concatenate((
                path_start_pts, path_line_offset,
                path_end_pts, path_line_offset), axis=-1),
            np.broadcast_to(self.path_line_clr, (n_lines, 3)))
    ...
    def get_viewer_pos(self) -> 'tuple[np.ndarray, np.ndarray] | None':
        ...
        env = self.sim.envs[self.env_idx]
        trf = self.gym.get_camera_transform(self.sim_handle, env.handle, env.cam_handles[self.bot_idx])

        if self.view == Interface.VIEW_CAMERA:
            pos_view = trf.p
            pos_target = pos_view + trf.r.rotate(Interface.OFFSET_CAM_AHEAD)

        else:
            pos_target = trf.p
            pos_view = pos_target + trf.r.rotate(Interface.OFFSET_FLW_BEHIND)
    ...
    def get_debug_info(self) -> str:
        ...
        if self.session.preview:
            ...
            p_obj = b_obj.softmax(-1).mul_(100.).cpu().numpy()
        ...
        return (
            ...
            'OBSERVATION\n'
            f'Avg. img. chan. | Hc: {obs_img[0]: .2f} | Hs: {obs_img[1]: .2f} | VS: {obs_img[2]: .2f}\n'
            f'                | D:  {obs_img[3]: .2f}\n'
    ...
    def get_rendered_images(self) -> 'tuple[np.ndarray, np.ndarray, np.ndarray]':
        seg = self.session.last_data['obs'][0][self.all_bot_idx, 5].long()

        if self.prev_idx == self.PREV_RGB:
            clr = self.session.last_data['obs'][0][self.all_bot_idx, -3:]
            clr = clr.permute(1, 2, 0).cpu().numpy() * 255.

        elif self.prev_idx == self.PREV_OBS:
            clr = self.session.last_data['obs'][0][self.all_bot_idx, :3]
            clr = (clr + 1.) / 2.
            clr = clr.permute(1, 2, 0).cpu().numpy() * 255.
            ...
            clr = torch.where(
                seg >= cfg.OBJ_CLS_OFFSET,
                seg - cfg.OBJ_CLS_OFFSET,
                cfg.CLR_CLS_UNSPEC)

            clr = torch.nn.functional.one_hot(clr, cfg.N_CLR_CLASSES)
            clr = torch.movedim(clr, -1, 0).cpu().numpy()
            clr = (clr[..., None] * self.RGB_CLASSES).sum(axis=0)

        if self.prev_idx == self.PREV_OBS:
            dep = self.session.last_data['obs'][0][self.all_bot_idx, 3].cpu().numpy() * 255.

        else:
            dep = self.session.last_data['obs'][0][self.all_bot_idx, 4].cpu().numpy() * 255.

        seg[seg >= cfg.OBJ_CLS_OFFSET] = cfg.ENT_CLS_OBJ

        seg = torch.nn.functional.one_hot(seg, cfg.N_ENT_CLASSES)
        seg = torch.movedim(seg, -1, 0).cpu().numpy()
        seg = (seg[..., None] * self.ENT_CLASSES).sum(axis=0)
    ...
    def get_visnet_images(self) -> 'tuple[np.ndarray, np.ndarray, np.ndarray]':
        ...
        hsvd = obs_img[self.all_bot_idx:self.all_bot_idx+1, :cfg.OBS_IMG_CHANNELS]
        ...
        ent_seg = (ent_probs[..., None] * self.ENT_CLASSES).sum(axis=0)
    ...
    def save_camera_images(self, reconstruct: bool = False) -> 'tuple[str, str, str]':
        ...
        for img, cam_type in zip(images, ('rgb', 'dep', 'seg')):
            ...
            imwrite(filename, (np.flip(img, axis=-1) if cam_type != 'dep' else img).astype(np.uint8))
        ...
        for event in self.gym.query_viewer_action_events(self.viewer):
            ...
            if cmd_key in self.OBS_KEYS:
                ...
                if cmd_key == 'change_preview':
                    if not cmd_press:
                        self.prev_idx = (self.prev_idx + (-1 if self.key_vec[-1] else 1)) % self.N_PREV_MODES
                        self.prev_reconstruct = self.prev_idx == self.PREV_VISNET
    ...
    def eval_line_update(self, after_eval: bool = True) -> bool:
    ...
    def sync_redraw(self, after_eval: bool = True):
        ...
        self.update_view(update_lines=self.eval_line_update(after_eval))
...
class Session(MazeTask):
    ...
    ARGS = [
        {'name': '--env_cfg', 'type': str, 'default': '1x1', 'help': 'Number/level config. of parallel environments.'},
        ...
        {'name': '--text_mode', 'type': int, 'default': cfg.TEXT_NONE, 'help': 'Texture selection or gen. method.'},
        {'name': '--aug_num', 'type': int, 'default': 1, 'help': 'Num. of aug. texture sets in rand. aug. modes.'},
        ...
        {'name': '--model_name', 'type': str, 'default': 'diablx340', 'help': 'Policy model name/ID string.'},
        {'name': '--visnet_dir', 'type': str, 'default': cfg.ASSET_DIR, 'help': 'VisNet model directory.'},
    ...
    def __init__(self, args: Namespace):
        ...
        self.visnet_dir = args.visnet_dir
    ...
    def post_step(
        ...
        if step_data is not None:
            ...
            # Reset passage graph for visual debugging
            if self.interface is not None and self.interface.eval_line_update() and not step_data['nrst'].all():
                self.interface.validator = MazeValidator(
                    self.sim.envs[self.interface.env_idx].sampler, self.sim.envs[self.interface.env_idx].data)
    ...
    def train(self):
        ...
        # Load state
        encoder_path = os.path.join(self.visnet_dir, 'visenc.pt')
        decoder_path = os.path.join(self.visnet_dir, 'vismlp.pt')
    ...
    def train_vis(self):
        ...
        if self.sim.text_mode == cfg.TEXT_SPLIT:
            state = torch.load(os.path.join(self.visnet_dir, 'visnet.pt'), map_location=self.ckpter.device)

            ref_visnet = VisNet().to(self.ckpter.device)
            ref_visnet.load_state_dict(state)
            model.visnet.load_state_dict(state)
        ...
        # Init. aux. task
        aux_task = VisionAuxTask(model.visnet, optimizer, ref_visnet)
