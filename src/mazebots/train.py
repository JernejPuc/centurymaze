...
from model import ActorCriticTemplate as ACT
from utils_torch import ActionDistrTemplate, ValueDistrTemplate
from utils_train import Batch, CheckpointTracker, ExperienceBuffer, SoftConstLRScheduler
...
class PPG:
    ...
    def __init__(
        ...
        self.model: ACT = ckpt_tracker.model
        ...
        self.stats = {
            ...
            'aux_adv_mean': new_zero_tensor(),
            'aux_adv_std': new_zero_tensor(),
    ...
    def run(self):
        ...
        for epoch_step in range(starting_step+1, self.n_epochs+1):
            ...
            for i in range(1, self.n_main_iters+1):
                ...
                obs, mem, nonrst = self.collect(obs, mem)

                self.label(obs, mem, nonrst, True)
            ...
            for i in range(1, self.n_aux_iters+1):
                ...
                self.label(obs, mem, nonrst, False)
                mem = self.update_aux()
    ...
    def label(
        ...
        with torch.no_grad():
            ...
            # Final values (future returns) in an episode are assumed to be zeros
            # Earlier returns are based (bootstrapped) on the model's estimates
            values = values * nonrst_mask
            ...
            if in_main:
                ...
                adv_mean, adv_std = self.aux_buffer.label(values, self.discount_factor, self.gae_lambda)

                self.stats['aux_adv_mean'] += adv_mean
                self.stats['aux_adv_std'] += adv_std

    def log(self, epoch_step: int):
        den_main = self.n_rollout_steps * self.n_main_iters
        den_aux = den_main * self.n_aux_iters

        # To keep loop simple
        self.stats['main_adv_mean'] *= den_main / self.n_main_iters
        self.stats['main_adv_std'] *= den_main / self.n_main_iters
        self.stats['aux_adv_mean'] *= den_aux / self.n_aux_iters
        self.stats['aux_adv_std'] *= den_aux / self.n_aux_iters
    ...
    def update_main_single(self, batches: 'list[Batch]', mem: 'tuple[Tensor, ...]') -> 'tuple[Tensor, ...]':
        stats = self.stats
        running_loss = 0.
        ...
        for batch in batches:
            ...
            value_loss = -self.value_weight * val.log_prob(batch['ret']).mean()

            # Entropy
            entropy = -self.entropy_weight * act.entropy().mean()

            # Total
            full_loss = policy_loss + value_loss + entropy
    ...
    def accel_main(self, batches: 'list[Batch]', mem: 'tuple[Tensor, ...]', inputs: 'list[Tensor]'):
        n_mem_items = len(mem)
        batch_ref = batches[0]

        # Resume input structure and relay as actual args.
        ...
        self.update_main_single_accel, self.accel_graphs['main'] = build_graph(
            update_main_single_accel,
            inputs,
            warmup_tensor_list=(),
            single_input=True)

        # Reset effects of multiple passes (2 warmup + 1 capture + 1 for out)
        # NOTE: Effect on params. is left as is, assuming small learning rate
        # Alternatively, learning rate could be divided too or set to 0.
        self.reward /= 4.
    ...
    def update_aux_single(self, batches: 'list[Batch]', mem: 'tuple[Tensor, ...]') -> 'tuple[Tensor, ...]':
        stats = self.stats
        running_loss = 0.
        ...
        for batch in batches:
            ...
            kl_div = act.kl_div(old_act).mean()
    ...
    def accel_aux(self, batches: 'list[Batch]', mem: 'tuple[Tensor, ...]', inputs: 'list[Tensor]'):
        n_mem_items = len(mem)
        batch_ref = batches[0]
        ...
        with cuda.stream(s):
            for _ in range(2):
                self.optimiser.zero_grad(set_to_none=True)
                self.update_aux_single(batches, mem)
        ...
        # Capture computational graph
        self.update_aux_single_accel, self.accel_graphs['aux'] = build_graph(
