...
class VisionAuxTask(AuxTask):
    ...
    WALL_CLR_IDX_OFFSET = len(cfg.COLOURS['neutral'])
    GOAL_CLR_IDX_OFFSET = len(cfg.COLOURS['neutral']) + len(cfg.COLOURS['wall'])
    ...
    def __init__(self, visnet: Module, optimizer):
    ...
    def get_clr_seg(self, ent_seg: Tensor) -> Tensor:
        ...
        clr_seg = torch.where(
            ent_seg >= cfg.WALL_CLS_OFFSET,
            ent_seg - cfg.WALL_CLS_OFFSET + self.WALL_CLR_IDX_OFFSET,
            cfg.SKY_CLR_IDX)

        clr_seg = torch.where(
            ent_seg >= cfg.OBJ_CLS_OFFSET,
            ent_seg - cfg.OBJ_CLS_OFFSET + self.GOAL_CLR_IDX_OFFSET,
            clr_seg)

        clr_seg = torch.where(
            ent_seg >= cfg.BOT_CLS_OFFSET,
            ent_seg - cfg.BOT_CLS_OFFSET + self.GOAL_CLR_IDX_OFFSET,
            clr_seg)

        clr_seg[ent_seg == cfg.PLD_CLS_OFFSET] = cfg.WHITE_CLR_IDX

        # Explicitly set const. clr. elements
        clr_seg[ent_seg == cfg.ENT_CLS_BOT] = cfg.WHITE_CLR_IDX
        clr_seg[(ent_seg == cfg.ENT_CLS_CLUTTER) | (ent_seg == cfg.ENT_CLS_FLOOR) | (ent_seg == cfg.ENT_CLS_WALL)] \
            = cfg.FLOOR_CLR_IDX

        # Revert to original segmentation tags
        ent_seg[(ent_seg >= cfg.WALL_CLS_OFFSET) & (ent_seg < cfg.OBJ_CLS_OFFSET)] = cfg.ENT_CLS_WALL
        ent_seg[(ent_seg >= cfg.OBJ_CLS_OFFSET) & (ent_seg < cfg.BOT_CLS_OFFSET)] = cfg.ENT_CLS_OBJ
        ent_seg[ent_seg >= cfg.BOT_CLS_OFFSET] = cfg.ENT_CLS_PAYLOAD
