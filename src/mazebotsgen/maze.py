...
class MazeConstructor:
    ...
    def __init__(
        ...
        self.n_side_divs = int(side_length / cfg.WALL_LENGTH)
        self.wall_halflength = cfg.WALL_LENGTH / 2.
    ...
    def init_candidate_points(self):
        ...
        n_spawn_cells = ceil(self.n_bots / cfg.N_CELL_DIVS ** 2)
        n_buffer_cells = min(self.n_side_divs**2 - 2, ceil((n_spawn_cells ** 0.5 + 2) ** 2))
        ...
        cell_sub_spacing = self.side_length / (cfg.N_CELL_DIVS * self.n_side_divs)

        offsets = np.linspace(-1., 1., cfg.N_CELL_DIVS) * (cell_spacing - cell_sub_spacing) / 2.
    ...
    def generate(self) -> MazeData:
        ...
        while True:
            ...
            block_mask, wall_mask, cell_wall_grid = self.prune_grid(graph_edges)
            ...
            obj_pts = self.sample_viable_points(
                ...
                self.n_goals, self.min_goal_spacing,
                None, 0.,
                cfg.OBJ_TO_WALL_GAP, cfg.MAX_OBJ_TO_WALL_DIST)
        ...
        wall_clr_idcs = self.rng.permutation(cfg.N_WALL_CLRS)

        # Assign block colours in clusters based on position
        # NOTE: Ignore warning: Empty clusters (not associated with any point) are auto-resolved with random points
        block_pts = self.cell_pt_grid[block_mask]

        cluster_centroids = (
            block_pts
            if len(block_pts) <= cfg.N_WALL_CLRS
            else kmeans2(block_pts, cfg.N_WALL_CLRS, seed=self.rng)[0])

        closest_centroid_idcs = np.argmin(np.linalg.norm(block_pts[None] - cluster_centroids[:, None], axis=-1), axis=0)
        block_clr_idcs = wall_clr_idcs[closest_centroid_idcs]

        cell_clr_idx_grid = np.full((self.n_side_divs,)*2, -1, dtype=np.int64)
        cell_clr_idx_grid[block_mask] = block_clr_idcs
    ...
    def prune_grid(self, graph_edges: ndarray) -> 'tuple[ndarray, ndarray, ndarray]':
        ...
        return block_mask, wall_mask, cell_wall_grid
...
class MazeValidator:
    ...
    def __init__(self, sampler: MazeConstructor = None):
        ...
        self.border_clr = np.array(cfg.COLOURS['wall'])[self.data.wall_clr_idcs[-1]]
