...
class MazeTask:
    ...
    def __init__(
        ...
        # Grid cell states
        self.max_delims = torch.from_numpy(
            max([env.sampler.open_delims for env in sim.envs], key=len)).to(device, dtype=torch.float32)
    ...
    async def async_reset_and_recolour(self):
        ...
        # Colouring action
        if self.bot_done_mask.any():
            set_clr = self.gym.set_rigid_body_color
            set_sid = self.sim.gym.set_rigid_body_segmentation_id

            for (env_handle, bot_handle) in self.sim.env_bot_handles[self.bot_done_mask.cpu().numpy()]:
                set_clr(env_handle, bot_handle, cfg.BOT_LOAD_IDX, gymapi.MESH_VISUAL, MazeEnv.NEUTRAL_CLR)
                set_sid(env_handle, bot_handle, cfg.BOT_LOAD_IDX, cfg.PLD_CLS_OFFSET)
    ...
    async def async_eval_state(self):
        ...
        min_bot_dist, near_src_prox, prox_sig_agg = self.get_prox_data(obj_diff, obj_dist, bot_ori)
    ...
    def get_prox_data(self, obj_diff: Tensor, obj_dist: Tensor, bot_ori: Tensor) -> 'tuple[Tensor, ...]':
        ...
        for env in self.sim.envs:
            ...
            # BxBx0|2, BxGx0|2 -> Bx(B+G)x0|2
            src_dist = torch.cat((bot_dist, obj_dist[env.bot_slice]), dim=1)
            src_diff = torch.cat((bot_diff, obj_diff[env.bot_slice]), dim=1)

            # Proximity weights via truncated inverse prop. characteristic
            sig_strength = norm_distance(src_dist, cfg.PROX_SIGNAL_RADIUS).mul_(2.)
