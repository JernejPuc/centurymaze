...
CMD_PRESETS: 'dict[str, list[str]]' = {
    'test': [BASE_CALL],
    'train_vis': [f'{BASE_CALL} --ctrl_mode 1 --headless 1 --n_envs {N_ENVS} --ep_duration {VIS_EP_DURATION}'],
    'train_rl': [BASE_CALL + ' --ctrl_mode 2 --headless 1'],
...
for agent_type, args in AGENT_TYPE_CONFIGS.items():
    for seed in SEEDS:
        ...
        CMD_PRESETS['train_all'].append(
            f'{BASE_CALL} --ctrl_mode 2 --headless 1 --n_envs {N_ENVS} --n_bots {N_BOTS} '
